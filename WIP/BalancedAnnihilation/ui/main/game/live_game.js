...
$(document).ready(function() {
    ...
    function CelestialViewModel(object) {
        ...
        if (self.thrust_control() && model.planetIndexToThrustControlMap[self.index()] !== 'friendly') {
            ...
            if (!model.isSpectator())
                model.doCustomAlert('Thrust control established').then(function() {
        ...
        self.isSelected = ko.computed(function() {
            return self.index() === model.selectedCelestialIndex()
        });
    ...
    function CelestialControlModel() {
        ...
        self.actionIsChangeOrbit = ko.computed(function() {
            return self.actionIndex() === 1;
        });
        self.validTargetPlanetIndexList = ko.observableArray([]).extend({
            withPrevious: true
        });
        ...
        self.sourcePlanetIndex = ko.observable(-1).extend({
            withPrevious: true
        });
        ...
        self.hasSource = ko.computed(function() {
            return self.sourcePlanetIndex() !== -1
        });
        self.hasSurfaceTarget = ko.observable(false);
        self.requireConfirmation = ko.observable(false);
        self.requireSurfaceTarget = ko.computed(function() {
            return self.actionIndex() == 2;
        });
        self.hasSurfaceTarget.subscribe(function(value) {
            if (value)
                self.requireConfirmation(true);
        });
        self.targetPlanetIndex = ko.observable(-1).extend({
            withPrevious: true
        });
        ...
        self.mousedownTargetPlanetIndex = ko.observable(-1).extend({
            withPrevious: true
        });
        ...
        self.selectedPlanetIndex = ko.observable(-1).extend({
            withPrevious: true
        });
        ...
        self.notActive = ko.computed(function() {
            return self.sourcePlanetIndex() === -1;
        });
        self.active = ko.computed(function() {
            return !self.notActive();
        });
        self.findingTargetSurfacePosition = ko.computed(function() {
            return self.targetPlanetIndex() !== -1;
        });
        self.findingTargetPlanet = ko.computed(function() {
            return self.sourcePlanetIndex() !== -1 && !self.findingTargetSurfacePosition();
        });
    ...
    function GameOptionModel(object) {
        ...
        self.isFFA = function() {
            return self.game_type() === '0'
        };
        self.isTeamArmy = function() {
            return self.game_type() === '1'
        };
        self.isGalaticWar = function() {
            return self.game_type() === 'Galactic War'
        };
    }
    ...
    function LiveGameViewModel() {
        ...
        self.reviewMode = ko.observable(false).extend({
            session: 'review_mode'
        });
        self.forceResumeAfterReview = ko.observable(false);
        self.gameOver = ko.observable(false).extend({
            session: 'game_over'
        });
        ...
        self.celestialControlActive = ko.computed(function() {
            return !self.celestialControlModel.notActive()
        });
        ...
        self.playerContactMap = ko.observable({}).extend({
            session: 'player_contact_map'
        });
        ...
        self.togglePinPlayerListPanel = function() {
            self.pinPlayerListPanel(!self.pinPlayerListPanel());
        };
        self.showPlayerListPanel = ko.computed(function() {
            return self.pinPlayerListPanel() && self.showPlayerViewModels()
        });
        // Spectator Panel pinning
        self.pinSpectatorPanel = ko.observable(true);
        self.togglePinSpectatorPanel = function() {
            self.pinSpectatorPanel(!self.pinSpectatorPanel());
        };
        self.showSpectatorPanel = ko.computed(function() {
            return self.pinSpectatorPanel() && self.showPlayerViewModels() && self.isSpectator()
        });
        ...
        self.updatePlayerVisionFlag = function(index) {
            ...
            self.send_message('change_vision_flags', {
                'vision_flags': self.playerVisionFlags()
            });
            engine.call('game.updateObservableArmySet', flags);
        };
        ...
        self.updatePlayerControlFlag = function(index) {
            ...
            self.playerVisionFlags.notifySubscribers();
            self.send_message('change_control_flags', {
                'control_flags': self.playerControlFlags()
            });
            self.send_message('change_vision_flags', {
                'vision_flags': self.playerVisionFlags()
            });
        ...
        self.startObserverMode = function() {
            ...
            /* Time */
        self.showTimeControls = ko.observable(false).extend({
            session: 'show_time_controls'
        });
        ...
        self.showOptionsBar = ko.computed(function() {
            return self.optionsBarIsOpen() && !self.showTimeControls()
        });
        self.showSelectionBar = ko.computed(function() {
            return !self.showOptionsBar() && !self.reviewMode() && self.celestialControlModel.notActive();
        });
        ...
        self.showLanding = ko.observable().extend({
            session: 'showLanding'
        });
        ...
        self.showMessage = ko.computed(function() {
            return self.showLanding() || self.inPlanetAnnihilateSequence() || self.inPlanetMoveSequence();
        });
        self.message = ko.observable().extend({
            session: 'lg_message'
        });
        self.confirming = ko.observable(false).extend({
            session: 'lg_confirming'
        });
        ...
        self.hasSelection = ko.computed(function() {
            return !!self.selection() && self.selection().spec_ids && !$.isEmptyObject(self.selection().spec_ids)
        });
        self.selectionModel = ko.observable(new SelectionViewModel({}));
        self.selectionList = ko.computed(function() {
            return self.selectionModel().list()
        });
        ...
        self.onSelectionDisplayClick = function(index, event, force_remove) {
                ...
                switch (option) {
                    ...
                    case 'add':
                        if (!double) return;
                        break; // Already in the selection
                    case '':
                        if (!double) {
                            if (types.length === 1)
                                return;
        ...
        self.showOrders = ko.computed(function() {
            return !self.showTimeControls() && self.hasSelection() && !self.showLanding() && !self.reviewMode() && self.celestialControlModel.notActive();
        });
        self.showBuildList = ko.computed(function() {
            return !self.showTimeControls() && !self.showLanding() && !self.reviewMode() && self.celestialControlModel.notActive();
        });
        ...
        self.parseSelection = function(payload) {
            ...
            if (selectionCanBuild) {
                ...
                modify_keybinds({
                    remove: ['build structure', 'build unit']
                });
        ...
        self.setup = function() {
            ...
            modify_keybinds({
                add: ['camera controls', 'gameplay', 'camera', 'hacks']
            });
        ...
        $holodeck.each(function() {
            ...
            var holodeck = new api.Holodeck($this, {}, primary ? function(hdeck) {
                hdeck.focus();
            } : undefined);
            if (primary) {
        ...
        self.showPips.subscribe(function(show) {
            ...
            _.delay(function() {
                _.forEach(self.pips, function(pip) {
                    pip.update();
                });
        ...
        self.updateGameLoading = function() {
            ...
            if (loadingPanel)
                loadingPanel.message('toggle', {
                    show: self.showGameLoading()
                });
        };
        self.showGameLoading.subscribe(function() {
            self.updateGameLoading();
        });
        self.showUberBar = ko.observable(false).extend({
            session: 'show_uber_bar'
        });

        function updateUberBarVisibility() {
            api.Panel.message('uberbar', 'visible', {
                'value': self.showUberBar()
            });
            api.Panel.message(gPanelParentId, 'live_game_uberbar', {
                'value': self.showUberBar()
            });
        }
                ...
                if (mdevent.button === 0) {
                    ...
                    input.capture(holodeck.div, function(event) {
                        ...
                        if (!model.allowCustomFormations() && (command === 'move' || command === 'unload')) {
                            ...
                            holodeck.unitBeginCommand(command, startx, starty).then(function(ok) {
                                dragging = ok;
                            });
                        } else if ((event.type === 'mouseup') && (event.button === 0)) {
                            input.release();
        ...
        self.showEconSharing.subscribe(function() {
            api.Panel.message('econ', 'showSharedResources', {
                'value': self.showEconSharing()
            });
        });
        ...
        self.setDiplomaticState = function(targetArmyid, state) {
            self.send_message('change_diplomatic_state', {
        ...
        self.isAlly = function(army) {
            if (!army || !self.player() || !self.player().diplomaticState || !self.player().diplomaticState[army.id])
                return;
            return army ? self.player().diplomaticState[army.id].state === 'allied' || self.player().diplomaticState[army.id].state === 'allied_eco' : false;
        };
        self.isAllyLocked = function(army) {
            if (!army || !self.player() || !self.player().diplomaticState || !self.player().diplomaticState[army.id])
                return
            var diplomatic_state = self.player().diplomaticState[army.id];
            return (diplomatic_state.state === 'allied' || diplomatic_state.state === 'allied_eco') && !diplomatic_state.mutable;
        };
            ...
            if (!self.isSpectator()) {
                ...
                if (!(army.stateToPlayer === 'allied' || army.stateToPlayer === 'allied_eco')) {
                    if (army.diplomaticState && army.diplomaticState[self.armyId()] && army.diplomaticState[self.armyId()].allianceRequest) {
                        var request = _.find(self.allianceRequestsReceived(), function(request) {
                            return request.id === army.id;
                        });
                        if (!request)
                            self.allianceRequestsReceived.push({
                                id: army.id
                            });
                    }
                }
        ...
        self.pauseSim = function() {
            self.send_message('control_sim', {
                paused: true
            });
        };
            ...
            if (msg.data.armies) {
                if ((msg.state !== 'replay') && (msg.state !== 'load_replay')) {
                    engine.call('execute', 'army_id', JSON.stringify({
                        ...
                        army_list: _.map(msg.data.armies, function(army) {
                            return army.id;
                        })
                    }));
                }
            ...
            switch (msg.state) {
                case 'landing':
                    ...
                    if (msg.data.client && msg.data.client.zones)
                        engine.call('execute', 'landing_zones', JSON.stringify({
                            landing_zones: msg.data.client.zones
                        }));
    ...
    handlers.unit_specs = function(payload) {
        ...
        _.forIn(model.unitSpecs, function(unit, id) {
            ...
            _.forEach(['build', 'projectiles'], function(element) {
                unit.canBuild |= _.some(unit[element] || [], function(target) {
    ...
    handlers.unit_data = function(payload) {
        ...
        _.forEach(payload.data, function(element, id) {
            var sicon = (element.sicon_override) ? element.sicon_override : siconFor(id);
    ...
    handlers.celestial_data = function(payload) {
        ...
        if (payload.planets && payload.planets.length) {
            ...
            model.celestialViewModels.push(new CelestialViewModel({
                isSun: true,
                index: payload.planets.length
            }));
        }
    ...
    handlers['query.action_keybinds'] = function() {
        return model.actionKeybinds();
    }
    handlers['query.action_state'] = function() {
        return model.actionBarState();
    }
